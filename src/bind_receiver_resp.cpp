 ...
 */
/// @file bind_receiver_resp.cpp
/// @brief Declares a bind_receiver_resp

#include "bind_receiver_resp.hpp"

/// @brief Default constructor.
/// note Sets all values to their defaults.
Smpp::BindReceiverResp::BindReceiverResp() :
...
Smpp::BindReceiverResp::BindReceiverResp(const Smpp::Uint8* b) :
    Response(CommandLength(MinLength),
              CommandId(CommandId::BindReceiverResp),
              CommandStatus(CommandStatus::ESME_ROK),
              SequenceNumber(1))
...
void
Smpp::BindReceiverResp::decode(const Smpp::Uint8* buff)
{
    Response::decode(buff);

    auto len = Response::command_length();
    Smpp::Uint32 offset = 16;
    const char* err = "Bad length in bind_receiver_resp";
    if(len < offset) {
        throw Error(err);
    }
