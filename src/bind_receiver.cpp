...
#include "bind_receiver.hpp"

/// @brief Default constructor.
/// @note Sets all parameters to their default values.
Smpp::BindReceiver::BindReceiver() :
...
Smpp::BindReceiver::BindReceiver(const Smpp::Uint8* b) :
    Request(CommandLength(MinLength),
    CommandId(CommandId::BindReceiver),
    SequenceNumber(1))
{
...
void
Smpp::BindReceiver::decode(const Smpp::Uint8* buff)
{
    Request::decode(buff);
