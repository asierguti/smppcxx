 ...
 */
/// @file bind_transmitter_resp.cpp
/// @brief Declares a bind_transmitter_resp

#include "bind_transmitter_resp.hpp"

/// @brief Default constructor.
/// note Sets all values to their defaults.
Smpp::BindTransmitterResp::BindTransmitterResp() :
...
Smpp::BindTransmitterResp::BindTransmitterResp(const Smpp::Uint8* b) :
    Response(CommandLength(MinLength),
              CommandId(CommandId::BindTransmitterResp),
              CommandStatus(CommandStatus::ESME_ROK),
              SequenceNumber(1))
...
const Smpp::Uint8*
Smpp::BindTransmitterResp::encode()
{
...
void
Smpp::BindTransmitterResp::decode(const Smpp::Uint8* buff)
{
    Response::decode(buff);

    auto len = Response::command_length();
    Smpp::Uint32 offset = 16;
    const char* err = "Bad length in bind_transmitter_resp";
    if(len < offset) {
        throw Error(err);
    }
