 ...
 */
/// @file enquire_link_resp.cpp
/// @brief Declares a enquire_link_resp

#include "enquire_link_resp.hpp"
...
void
Smpp::EnquireLinkResp::decode(const Smpp::Uint8* buff)
{
    Response::decode(buff);

    auto len = Response::command_length();
    Smpp::Uint32 offset = 16;
    const char* err = "Bad length in enquire_link_resp";
    if(len < offset) {
        throw Error(err);
    }
