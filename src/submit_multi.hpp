 ...
 */
#ifndef __SMPP_SUBMIT_MULTI_HPP__
#define __SMPP_SUBMIT_MULTI_HPP__

/// @file submit_multi.hpp
/// @brief An SMPP submit_multi PDU.
...
namespace Smpp {
    ...
    /// @class SubmitMulti
    /// @brief Definition of an SMPP submit_multi, derived from a Request.
    class SubmitMulti : public Request {
        ServiceType service_type_;
        SmeAddress source_addr_;
        MultiDestinationAddresses destination_addr_;
        EsmClass esm_class_;
      ...
      public:
        /** submit_multi minimum length in octets */
        const static int MinLength = 31;

        /** @brief Default constructor. */
        SubmitMulti();

        /** @brief Constructor requiring all mandatory parameters except the
         * destination addresses. These are added individually. */
        SubmitMulti(
        ...
        SubmitMulti(const Smpp::Uint8* b);
        ...
        /// @brief Add a destination address using an SmeAddress object.
        /// @param p The new destination address.
        void destination_addr(const SmeAddress& p) {
            Header::update_length(destination_addr_.add(p));
        }

        /// @brief Add a destination address using a distribution list name.
        /// @param p The distribution list.
        void destination_addr(const Smpp::String& p) {
            Header::update_length(destination_addr_.add(p));
        }
        ...
        const MultiDestinationAddresses& destination_addr() const {
