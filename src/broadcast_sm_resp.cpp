 ...
 */
/// @file broadcast_sm_resp.cpp
/// @brief Declares a broadcast_sm_resp

#include "broadcast_sm_resp.hpp"

/// @brief Default constructor.
/// note Sets all values to their defaults.
Smpp::BroadcastSmResp::BroadcastSmResp() :
...
Smpp::BroadcastSmResp::BroadcastSmResp(const Smpp::Uint8* b) :
    Response(CommandLength(MinLength),
              CommandId(CommandId::BroadcastSmResp),
              CommandStatus(CommandStatus::ESME_ROK),
              SequenceNumber(1))
...
void
Smpp::BroadcastSmResp::decode(const Smpp::Uint8* buff)
{
    Response::decode(buff);

    auto len = Response::command_length();
    Smpp::Uint32 offset = 16;
    const char* err = "Bad length in broadcast_sm_resp";
    if(len < offset) {
        throw Error(err);
    }
